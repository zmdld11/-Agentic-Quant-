...
.venv/Scripts/python.exe main.py s1        # 2. 跑第一个回测：双均线策略
# 结果：终端打印报告，results/s1_ma_cross/ 下生成 净值图 + 交易流水 + 报告
.venv/Scripts/python.exe main.py s1 --short 10 --long 60   # 3. 换参数玩
```
...
quant/             核心包，建议按此顺序读：
  ...
  engine.py         回测引擎：T+1、涨跌停、一手100股、佣金/印花税/滑点、防前视偏差
  metrics.py        绩效：年化/波动/夏普/最大回撤/卡玛/回合胜率
  plotting.py       净值曲线、回撤面积、买卖点标注
...
| 4 | 动量ETF轮动、均值回归、网格；参数扫描 + 样本外测试（过拟合实操） | ⬜ 下一步 |
| 5 | 横截面因子选股（全市场排序、IC 检验）/ 评估 qlib / 模拟盘 | ⬜ 远期 |
